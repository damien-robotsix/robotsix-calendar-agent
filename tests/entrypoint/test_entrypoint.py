...
from robotsix_calendar_agent.entrypoint import (
    COMPONENT_KINDS,
...
class TestResponderInertWhenDisabled:
    def test_responder_not_created_when_disabled(self) -> None:
        ...
        with patch.dict(os.environ, {}, clear=True):
            s = Settings()
            result = entrypoint._build_component_responder(s)
    ...
    def test_responder_not_created_when_sdk_missing(self) -> None:
        ...
        with (
            patch.dict(os.environ, env, clear=True),
            patch(
                "robotsix_calendar_agent.entrypoint.importlib.util.find_spec",
                return_value=None,
            ),
        ):
            s = Settings()
